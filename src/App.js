...
let schema = yup.object().shape({
  ...
  zip: yup
    .string()
    .notRequired()
    .min(5, "Zip can only be 5 characters.")
    .max(5, "Zip can only be 5 characters.")
    .nullable()
  ...
  return (
    ...
    >
      ...
      >
        ...
        />
        <TextField
          inputRef={zipRef} // NOT ref - inputRef in MUI
          id="zip"
          label="Zip Code"
          type="number"
          variant="standard"
        ...
        {validationResults.map((validationMessage) => {
          return (
            <p style={{ margin: 3, color: "firebrick" }}>{validationMessage}</p>
          );
        })}
      </form>
    </Box>
